 ...
 */
function unrefRpcServerHandles(port) {
...
export async function startChain(hre) {
    if (!(await isPortAvailable(NOX_LOCAL_PORT))) {
        throw new Error(`[nox] Port ${NOX_LOCAL_PORT} is already in use. A Hardhat node ` +
            `(or another process) is already bound to it. Stop it and re-run ` +
            `\`hardhat test\`.`);
    }
    const server = await hre.network.createServer({ network: NOX_HOST_NETWORK }, "0.0.0.0", NOX_LOCAL_PORT);
