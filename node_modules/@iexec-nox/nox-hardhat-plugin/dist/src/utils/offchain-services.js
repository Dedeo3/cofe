...
import { ALL_SERVICES, COMPOSE_OPTS, HANDLE_GATEWAY_CONTAINER_PORT, HANDLE_GATEWAY_HOST_PORT_ENV, HANDLE_GATEWAY_SERVICE, } from "../nox-config.js";
import { assertDockerDaemonRunning } from "./docker.js";
/** Run a docker-compose operation, rethrowing failures with a clean message. */
async function runComposeWithCleanErrors(action, op) {
    try {
        return await op();
    }
    catch (error) {
...
export async function startOffchainServices() {
    ...
    const { data } = await runComposeWithCleanErrors("start", () => port(HANDLE_GATEWAY_SERVICE, HANDLE_GATEWAY_CONTAINER_PORT, COMPOSE_OPTS));
    if (!data.port) {
...
//# sourceMappingURL=offchain-services.js.map