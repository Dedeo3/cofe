...
import { NotYetComputedHandleError, UnknownHandleError, } from '../utils/error.js';
import { isHexString } from '../utils/hex.js';
import { retry } from '../utils/retry.js';
import { generateRsaKeyPair, exportRsaPublicKey, rsaDecrypt, exportRsaPrivateKey, importRsaPrivateKey, } from '../utils/rsa.js';
import { handleToChainId, handleToSolidityType, isUniqueHandle, } from '../utils/types.js';
import { assertRequiredParams } from '../utils/validators.js';
import { viewACL } from './viewACL.js';
export async function decrypt({ handle, apiService, blockchainService, subgraphService, storageService, config, }) {
    ...
    const isViewer = await blockchainService.readContract(config.smartContractAddress, IS_VIEWER_ABI, [handle, userAddress]);
    if (!isViewer) {
    ...
    const getHandleCryptoMaterial = async () => {
        ...
        if (response.status === 404) {
            if (!handleVerifiedToExist && !isUniqueHandle(handle)) {
                await viewACL({ subgraphService, blockchainService, handle }).catch((error) => {
    ...
    const response = await retry(getHandleCryptoMaterial, {
        delay: 1000,
        backoff: 2,
        maxRetries: 3,
    ...
    if (response.status !== 200 ||
        ...
        response.data === null ||
        !isHexString(response.data?.ciphertext) ||
        !isHexString(response.data?.iv, 12) ||
        !isHexString(response.data
...
async function generateDecryptionMaterial({ userAddress, chainId, smartContractAddress, blockchainService, }) {
    ...
    const spkiHexRsaPubKey = await exportRsaPublicKey(rsaKeyPair).catch((error) => {
