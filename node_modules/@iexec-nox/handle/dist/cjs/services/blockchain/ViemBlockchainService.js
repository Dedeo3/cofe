"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.ViemBlockchainService = void 0;
exports.isViemWalletClient = isViemWalletClient;
exports.isViemSmartAccount = isViemSmartAccount;
const format_js_1 = require("../../utils/format.js");
function isViemWalletClient(object) {
    return (!!object &&
        typeof object === 'object' &&
        'getChainId' in object &&
        typeof object.getChainId === 'function' &&
        'getAddresses' in object &&
        typeof object.getAddresses === 'function' &&
        'signTypedData' in object &&
        typeof object.signTypedData === 'function');
}
class WalletClientAdapter {
    walletClient;
    constructor(walletClient) {
        this.walletClient = walletClient;
    }
    async getClient() {
        return this.walletClient;
    }
    async getChainId() {
        return this.walletClient.getChainId();
    }
    async getAddress() {
        const addresses = await this.walletClient.getAddresses();
        const address = addresses[0];
        if (!address) {
            throw new Error('No connected account');
        }
        return address;
    }
    async signTypedData(data) {
        return this.walletClient.signTypedData({
            ...data,
            account: this.walletClient.account ?? (await this.getAddress()),
        });
    }
}
function isViemSmartAccount(object) {
    return (!!object &&
        typeof object === 'object' &&
        'type' in object &&
        object.type === 'smart' &&
        'getAddress' in object &&
        typeof object.getAddress === 'function' &&
        'signTypedData' in object &&
        typeof object.signTypedData === 'function' &&
        'client' in object &&
        !!object.client &&
        typeof object.client === 'object' &&
        'chain' in object.client &&
        !!object.client.chain &&
        typeof object.client.chain === 'object' &&
        'id' in object.client.chain &&
        typeof object.client.chain.id === 'number');
}
class SmartAccountAdapter {
    smartAccount;
    constructor(smartAccount) {
        this.smartAccount = smartAccount;
    }
    async getClient() {
        return this.smartAccount.client;
    }
    async getChainId() {
        const chainId = this.smartAccount.client.chain?.id;
        if (chainId === undefined) {
            throw new Error('SmartAccount client is not connected to a chain');
        }
        return chainId;
    }
    async getAddress() {
        return this.smartAccount.getAddress();
    }
    async signTypedData(data) {
        return this.smartAccount.signTypedData(data);
    }
}
class ViemBlockchainService {
    static viemModule = null;
    static async getViemModule() {
        this.viemModule ??= await Promise.resolve().then(() => __importStar(require('viem')));
        return this.viemModule;
    }
    adapter;
    constructor(client) {
        if (isViemSmartAccount(client)) {
            this.adapter = new SmartAccountAdapter(client);
        }
        else if (isViemWalletClient(client)) {
            this.adapter = new WalletClientAdapter(client);
        }
        else {
            throw new TypeError('Unsupported client. Expected a viem WalletClient instance connected to an account.');
        }
    }
    async getChainId() {
        try {
            const chainId = await this.adapter.getChainId();
            return chainId;
        }
        catch (error) {
            throw new Error('Failed to get chain ID', { cause: error });
        }
    }
    async getBlockNumber() {
        try {
            const { publicActions } = await ViemBlockchainService.getViemModule();
            const clientToExtend = await this.adapter.getClient();
            const publicClient = clientToExtend.extend(publicActions);
            return Number(await publicClient.getBlockNumber());
        }
        catch (error) {
            throw new Error('Failed to get block number', { cause: error });
        }
    }
    async getAddress() {
        try {
            return await this.adapter.getAddress();
        }
        catch (error) {
            throw new Error('Failed to get address', { cause: error });
        }
    }
    async readContract(contractAddress, abiFunctionFragment, parameters) {
        try {
            const { publicActions } = await ViemBlockchainService.getViemModule();
            const clientToExtend = await this.adapter.getClient();
            const publicClient = clientToExtend.extend(publicActions);
            return (await publicClient.readContract({
                address: contractAddress,
                abi: [abiFunctionFragment],
                functionName: abiFunctionFragment.name,
                args: parameters,
            }));
        }
        catch (error) {
            throw new Error(`Failed to read contract at ${contractAddress} (method: ${abiFunctionFragment.name}, parameters: ${(0, format_js_1.safeJsonStringify)(parameters)})`, {
                cause: error,
            });
        }
    }
    async signTypedData(data) {
        try {
            const signature = await this.adapter.signTypedData({
                domain: data.domain,
                types: data.types,
                primaryType: data.primaryType,
                message: data.message,
            });
            return signature;
        }
        catch (error) {
            throw new Error('Failed to sign typed data', { cause: error });
        }
    }
    async verifyTypedData(data, signature) {
        try {
            const { recoverTypedDataAddress } = await ViemBlockchainService.getViemModule();
            return await recoverTypedDataAddress({
                domain: data.domain,
                types: data.types,
                primaryType: data.primaryType,
                message: data.message,
                signature: signature,
            });
        }
        catch (error) {
            throw new Error('Failed to verify typed data', { cause: error });
        }
    }
}
exports.ViemBlockchainService = ViemBlockchainService;
//# sourceMappingURL=ViemBlockchainService.js.map